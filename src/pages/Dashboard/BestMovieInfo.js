...
function BestMovieInfo() {
    const {id}=useParams();
    const [gelenFilm,setGelenFilm]=useState({});
   
    useEffect(()=>{
     fetch(`http://localhost:3003/bestMovies/${id}`)
     .then(res=>res.json())
     .then(data=>setGelenFilm(data))
    },[])
   console.log("bestmovieInfo Id: ",id)
      ...
      <div class="movieInfo-container">
       ...
       <img src={gelenFilm.Poster} alt='poster'></img>
        ...
        <h4>{gelenFilm.Genre}</h4>
          ...
          <h4 style={{"width":"100px","height":"34px","borderRadius":"20px","border":"1px solid","display":"flex","justifyContent":"center","alignItems":"center"}}>{gelenFilm.imdbRated}</h4>
          </div>
          <div>
             <h6 style={{"fontFamily":"Trebuchet MS"}}>{gelenFilm.Description}</h6>
          </div>
        </div>
